...
public class NaiveBayes extends AbstractClassifier {
    ...
    public void localLearningMethod(Instance inst, int idx){
        this.observedClassDistribution.addToValue((int) inst.classValue(), inst.weight());
        int instAttIndex = modelAttIndexToInstanceAttIndex(idx, inst);
        AttributeClassObserver obs = this.attributeObservers.get(idx);
        if (obs == null) {
            obs = inst.attribute(instAttIndex).isNominal() ? newNominalClassObserver()
                    : newNumericClassObserver();
            this.attributeObservers.set(idx, obs);
        }
        obs.observeAttributeClass(inst.value(instAttIndex), (int) inst.classValue(), inst.weight());

    }

    public double[] localGetVotesForInstance(Instance inst, int idx) {
        return localDoNaiveBayesPrediction(inst, idx, this.observedClassDistribution,
                this.attributeObservers);
    }

    public static double[] localDoNaiveBayesPrediction(Instance inst, int idx,
        ...
        for (int classIndex = 0; classIndex < votes.length; classIndex++) {
            votes[classIndex] = observedClassDistribution.getValue(classIndex)
                    / observedClassSum;
            int instAttIndex = modelAttIndexToInstanceAttIndex(idx, inst);
            AttributeClassObserver obs = attributeObservers.get(idx);
            if ((obs != null) && !inst.isMissing(idx)) {
